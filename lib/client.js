...
class MatrixClient extends _typedEventEmitter.TypedEventEmitter {
  ...
  async getEventTimeline(timelineSet, eventId) {
    ...
    this.processBeaconEvents(timelineSet.room, events); // There is no guarantee that the event ended up in "timeline" (we might have switched to a neighbouring
  ...
  * @return {Object} the response, with chunk, prev_batch and, next_batch.
  ...
  processBeaconEvents(room, events) {
    ...
    const beaconEvents = events.filter(event => _beacon.M_BEACON.matches(event.getType()));
